...
<body>
    ...
    <div class="container"> <br>
        ...
        <div class="row">
            <div class="col">
                <h2>Blog Posts</h2> <br>
                <button type="button" class="btn btn-success" onclick="location.href='addblogpost.php'">Add Blogpost</button>
                <?php
                if (isset($_SESSION['loginmessage'])) {
                    unset($_SESSION['loginmessage']);
                ?>

                    <div class="sucmsg"><label class="sucdisplay"> </label></div>
                    <script>
                        $('.sucmsg').html("<label class='sucdisplay'>Logged in successfully</label>");
                        $(".sucdisplay").fadeOut(5000);
                    </script>
                <?php } ?>
                <hr>
            </div>
            ...
            <tbody>
                ...
                while ($category = mysqli_fetch_array($result)) {  ?>
                    <tr>
                        ...
                        <td><a href="" class="delete" data-id='<?php echo $id = $category['id']; ?>' id="delete"><i class="fa fa-trash" aria-hidden="true"></i></a></td>
                    </tr>
                <?php  } ?>
            </tbody>
        </table>
        ...
        $pagLink = "";
        if ($page >= 2) {
            echo "<a href='blogposts.php?page=" . ($page - 1) . "'>  Prev </a>";
        }

        for ($i = 1; $i <= $number_of_page; $i++) {
            if ($i == $page) {
                $pagLink .= "<a class = 'active' href='blogposts.php?page="
                    . $i . "'>" . $i . " </a>";
            } else {
                $pagLink .= "<a href='blogposts.php?page=" . $i . "'>   
													" . $i . " </a>";
            }
        };
        echo $pagLink;
        ...
        if ($page < $number_of_page) {
            echo "<a href='blogposts.php?page=" . ($page + 1) . "'>  Next </a>";
        }
        ?>
